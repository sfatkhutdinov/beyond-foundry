...
interface ConfigUrls {
    ...
    alwaysPreparedSpells: (classId: number, classLevel: number, campaignId?: string, spellListIds?: number[]) => string;
    alwaysKnownSpells: (classId: number, classLevel: number, campaignId?: string, spellListIds?: number[], backgroundId?: number | null) => string;
    itemsAPI: (campaignId?: string) => string;
    monstersAPI: (skip: number, take: number, search?: string, homebrew?: boolean, homebrewOnly?: boolean, sources?: number[]) => string;
...
const CONFIG: Config = {
    ...
    urls: {
        ...
        characterUrl: (characterId: string) => `${CONFIG.urls.baseUrl}/character/${characterId}?includeCustomItems=true`,
        ...
        alwaysPreparedSpells: (classId: number, classLevel: number, campaignId?: string, spellListIds: number[] = []) => {
            let campaign = "";
            let spellLists = "";
        ...
        alwaysKnownSpells: (classId: number, classLevel: number, campaignId?: string, spellListIds: number[] = [], backgroundId: number | null = null) => {
            let campaign = "";
            let spellLists = "";
        ...
        monstersAPI: (skip: number, take: number, search: string = "", homebrew: boolean = false, homebrewOnly: boolean = false, sources: number[] = []) => {
            let sourceSearch = sources.reduce((previous: string, current: number) => previous + `&sources=${current}`, "");
