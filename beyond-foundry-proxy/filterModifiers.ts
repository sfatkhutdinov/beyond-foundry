 ...
 */
function extractInfo(arr: { friendlyTypeName: string; friendlySubtypeName: string }[]): string[] {
  return arr.map((e: { friendlyTypeName: string; friendlySubtypeName: string }) => `${e.friendlyTypeName} (${e.friendlySubtypeName})`);
}
 ...
 */
function getClassFeatures(cls: any, classLevel = 20): any[] {
  if (
 ...
 * @param {*} classLevel level requirement up to which the class features should be extracted
 */
function getClassModifiers(data: any, classFeatures: any[], isStartingClass = false): any[] {
  const modifiers = data.modifiers.class.filter((classModifier: any) => {
    ...
    if (feature !== undefined) {
      ...
      console.log(
        `${isFeatureAvailable ? "  [  AVAIL]" : "  [UNAVAIL]"} Modifier found: ${classModifier.friendlyTypeName} (${classModifier.friendlySubtypeName})`
      );
...
function getClassOptionModifiers(data: any): any[] {
  ...
  const modifiers = data.modifiers.class.filter((classModifier: any) => {
    ...
    if (feature === undefined) {
      console.log(
        `  [  EXTRA] Modifier found: ${classModifier.friendlyTypeName} (${classModifier.friendlySubtypeName})`
      );
      return true;
    }
    return false;
